...
def extract_paths(element, parent_path=""):
    """ Recursively extract paths for each field in the XML tree, ensuring no duplicate paths for the same field """
    paths = []
...
def xml_to_csv(xml_file_path, output_csv):
    """ Convert XML file to CSV with field names and full paths, avoiding duplicate field-path combinations """
    # Parse XML from the file
    tree = ET.parse(xml_file_path)
    root = tree.getroot()
    ...
    # Use a set to track unique (field, path) pairs
    seen = set()
    ...
    with open(output_csv, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writerow(['Field Name', 'Path'])  # Write header
        for field_name, path in paths:
            # Only write if this field-path pair hasn't been seen before
            if (field_name, path) not in seen:
                writer.writerow([field_name, path])
                seen.add((field_name, path))
